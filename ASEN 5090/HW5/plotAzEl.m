...
%       09-2024   : Y. Wang edited to take rows of input and only label
%                   the PRN for the last epoch
%
...
for row = 1:size(xx,1)
    ...
    if row==1
        q = plot(xx(row,~isnan(xx(row,:))),yy(row,~isnan(xx(row,:))),'*k','MarkerSize',6);
    else
        hold on;
        q = plot(xx(row,:),yy(row,:),'.k','MarkerSize',6);
    end
end

% Place satellite PRN numbers with satellite position
for i = find(~isnan(svs(row,:)))
    text(xx(row,i)+3,yy(row,i),int2str(svs(row,i)));
end
