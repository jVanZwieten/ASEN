...
% "zero order hold".
% P. Axelrad - changed terminology/usage from SV to PRN 9/12
...
if (max(prn)>37) || (min(prn)<1) || (min(size(prn))~=1)
	error('prn must be a row or column vector with integers between 1 and 37\n')
end
...
% force integers
testint=round(prn)-prn;
if testint ~= 0 
	warning('non-integer value entered for prn, rounding to closest integer\n');
	prn = round(prn);
end
